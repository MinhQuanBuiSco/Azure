...
# ──────────────────────────────────────────────────────
# Blog 7 — API Gateway: Full Setup
#
# Creates:
#   1. Same 4 AD app registrations as Blog 6
#   2. Same 3 test users with role assignments
#   3. Terraform: RG + ACR + 3 Container Apps + APIM
...
echo ""
echo "==> Blog 7: API Gateway Setup"
echo ""

# ACR name (alphanumeric only, globally unique)
if [[ -z "${ACR_NAME:-}" ]]; then
  read -rp "   ACR name (alphanumeric, globally unique, e.g. blog07acr): " ACR_NAME
fi
# APIM name (globally unique)
if [[ -z "${APIM_NAME:-}" ]]; then
  read -rp "   APIM name (globally unique, e.g. blog07-apim): " APIM_NAME
fi
# Publisher email
...
ADMIN_ROLE_ID="a1b2c3d4-0007-0007-0007-000000000001"
EDITOR_ROLE_ID="a1b2c3d4-0007-0007-0007-000000000002"
READER_ROLE_ID="a1b2c3d4-0007-0007-0007-000000000003"
AUDIT_WRITE_ROLE_ID="a1b2c3d4-0007-0007-0007-000000000010"

# ════════════════════════════════════════════════════════
# PHASE 1: Azure AD Setup (same as Blog 6)
# ════════════════════════════════════════════════════════
...
project_name        = "blog07-apigw"
resource_group_name = "rg-blog07-api-gateway"
...
echo ""
echo "═══════════════════════════════════════════════════"
echo "  Blog 7: API Gateway — Setup Complete!"
echo "═══════════════════════════════════════════════════"
echo ""
echo "  Azure AD:"
...
echo "    3. Sign in → create tasks → verify Notify + Audit"
echo "    4. Check APIM portal for request metrics"
echo ""
echo "  Cost warning:"
